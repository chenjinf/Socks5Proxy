...
namespace qcutil
{
	...
	vector<wstring> GetAllDriverLetter()
	{
		vector<wstring> AllDriverLeterList;

		DWORD dwLen = GetLogicalDriveStringsW(0, NULL);
		WCHAR *pDriver = new WCHAR[dwLen];
		memset(pDriver, 0, dwLen);
		GetLogicalDriveStringsW(dwLen, pDriver);
		...
		{
			//	Output(L"%s %s \r\n", pDriver + i * 4, strType);
			AllDriverLeterList.push_back(pDriver + i * 4);
		}
		delete[] pDriver;
		pDriver = NULL;
		return AllDriverLeterList;
	}
