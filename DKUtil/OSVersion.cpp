...
namespace qcutil
{
	...
	std::string GetWindowsVersion(OSVERSIONINFOEXA* pOsVer, SYSTEM_INFO* pSysInfo)
	{
		std::string     winver;
	...
	bool IsServer2016()
	{
		...
		BOOL Ret = !VerifyVersionInfoW( &osvi, VER_PRODUCT_TYPE, dwlConditionMask );
		return ((osvi.dwMajorVersion >= 10 && osvi.dwMinorVersion >= 0) && Ret);
	}
	typedef NTSTATUS(NTAPI* fnRtlGetVersion)(PRTL_OSVERSIONINFOW lpVersionInformation);
	bool GetOSVerRTL(OUT RTL_OS_VER& osv)
	{
		...
		{
			osv.wMajorVersion = LOWORD(verInfo.dwMajorVersion);
			osv.wMinorVersion = LOWORD(verInfo.dwMinorVersion);
			osv.wServicePackMajor = verInfo.wServicePackMajor;
			return true;
		}
		return false;
	}
