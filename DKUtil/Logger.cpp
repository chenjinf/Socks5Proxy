...
Logger::HelperW Logger::getHelperW(const char* component, Logger::Level level, const char* className, const char* fileName, int line)
{
	return Logger::HelperW(component, level, className, fileName, line);
}
...
Logger::HelperW::HelperW(const char* component, 
	Level level, 
...
void Logger::HelperW::operator()(const std::wstring& message)
{
	Logger::getInstance()->log(_component, _level, _className, String::fromStdWString(message), _fileName, _line);
}

void Logger::HelperW::operator()(const wchar_t* format, ...)
{
	const int count = 1024;
	wchar_t buffer[count] = L"";
	va_list ap;
	va_start(ap, format);
	_vsnwprintf_s(buffer, _countof(buffer) - 1, _TRUNCATE, format, ap);
	buffer[count-1] = 0;
	va_end(ap);
	Logger::getInstance()->log(_component, _level, _className, String::fromStdWString(buffer), _fileName, _line);
}
