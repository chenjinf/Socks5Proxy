...
void CMD5Checksum::Final(OUT BYTE* OutBuf, ULONG cbOutBufLen)
{
	//Save number of bits
	BYTE Bits[8];
	DWordToByte( Bits, m_nCount, 8 );
	...
	//Store final state in 'lpszMD5'
	const int nMD5Size = 16;
	if (cbOutBufLen < nMD5Size)
	{
		return;
	}

	DWordToByte( OutBuf, m_lMD5, nMD5Size );
}
...
void CMD5Checksum::GetMD5(char* pBuffer, unsigned long nleng, OUT BYTE* OutBuf, ULONG cbOutBufLen)
{
	//参数效验
	if (pBuffer == NULL || nleng <= 0)
		return;
	try
	{ 
		CMD5Checksum MD5Checksum;   //checksum object    
		MD5Checksum.Update((BYTE*)pBuffer, nleng);
		MD5Checksum.Final(OutBuf, cbOutBufLen);
	}
	//report any file exceptions in debug mode only
	catch ( ... )
	{
